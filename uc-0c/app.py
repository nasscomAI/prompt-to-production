...
def load_dataset(input_path: str):
    ...
    null_rows = [r for r in rows if not r.get("actual_spend") or r.get("actual_spend").strip() == ""]
    return rows, null_rows


def compute_growth(rows: list, ward: str, category: str, growth_type: str) -> list:
    """Compute period-over-period growth for specific ward and category."""
    if not growth_type:
        raise ValueError("Error: --growth-type must be explicitly provided (e.g. 'MoM'). Refusing to assume formula silently.")

    if growth_type.upper() != "MOM":
        raise NotImplementedError(f"Growth type '{growth_type}' is not supported yet. Currently supported: 'MoM'.")
...
def main():
    ...
    print(f"Loaded {len(rows)} records. Found {len(null_rows)} total null rows across dataset.")
