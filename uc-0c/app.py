...
def compute_growth(data, ward, category, growth_type, output_path):
    ...
    for row in filtered:
        ...
        if not spend_str:
            note = row.get('notes', 'No data').strip() if row.get('notes') else 'No data'
            results.append({
                'period': period,
                'actual_spend': 'NULL',
                'mom_growth': 'NULL',
                'formula': 'Cannot compute - data missing',
                'flag': f'NULL_FLAGGED ({note})'
            })
            continue
        try:
            ...
            if prev_spend is not None:
                ...
                formula = f"(({spend:.1f} - {prev_spend:.1f}) / {prev_spend:.1f}) * 100 = {sign}{growth:.1f}%"
                mom_growth = f"{sign}{growth:.1f}%"
            else:
                mom_growth = 'N/A (baseline)'
                formula = 'First period in range - no prior data'
            results.append({
                'period': period,
