...
skills:
  ...
  - name: compute_growth
    description: Calculates growth (MoM or YoY) for a specific ward and category, returning a table that includes the specific formula used for each row.
    ...
    error_handling: Refuse and report if the growth type is unspecified or if the requested ward/category combination is not found.
