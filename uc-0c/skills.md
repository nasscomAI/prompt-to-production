...
skills:
  ...
  - name: compute_growth
    ...
    error_handling: >
      If the ward or category is not found in the data, exit with an error showing available options.
      If growth type is not specified or not "MoM", refuse and prompt the user.
      Never interpolate, average, or zero-fill null values; they must remain 'NULL' with growth marked 'N/A' and the flag column populated with the note.
      Periods must be sorted chronologically (YYYY-MM) before processing.
