...
skills:
  ...
  - name: batch_classify_complaint
    ...
    error_handling: If the input is missing or a row cannot be parsed, report the failure and skip invalid rows while preserving valid row order when possible but it should not fail the entire process.