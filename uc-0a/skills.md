skills:
  ...
  - error_handling: >
  - i) If description is missing, empty, or invalid, returns category "Other",
  - ii) priority "Low", reason indicating invalid input, and flag "NEEDS_REVIEW".
  - iii) If multiple or no categories match, sets flag to NEEDS_REVIEW.
  ...
  - error_handling: >
  - i) Skips or safely handles malformed rows without crashing.
  - ii) Ensures output file is always generated even if some rows fail.
  - iii) Missing fields are handled with default values and flagged as NEEDS_REVIEW.
