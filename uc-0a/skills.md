...
- name: batch*classify
  ...
  input: >
  Path to an input CSV (../data/city-test-files/test*[city].csv) with
  15 complaint rows per city; category and priority*flag columns are
  stripped from this file.
  output: >
  Path to an output CSV (uc-0a/results*[city].csv) containing every
