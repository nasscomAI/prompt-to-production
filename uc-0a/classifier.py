...
def classify_complaint(row: dict) -> dict:
    ...
    if any(re.search(r"\b" + re.escape(k) + r"\b", desc) for k in ["pothole"]):
        ...
        reason = f"Identified as Heat Hazard citing '{next(k for k in ['melting', 'heat', 'temperature', 'bubbling', 'sun'] if re.search(r'\b' + re.escape(k) + r'\b', desc))}'."
        ...
        reason = f"Refers to '{next(k for k in ['heritage', 'ancient', 'old city', 'step well'] if re.search(r'\b' + re.escape(k) + r'\b', desc))}' area."
