...
# Each keyword list is sorted longest-first so that more-specific compound
# phrases (e.g. "overflowing garbage bins") are matched before shorter, more
# ambiguous ones (e.g. "overflow"). This prevents cross-category collisions.
_RAW_CATEGORY_KEYWORDS: dict[str, list[str]] = {
    "Pothole": [
    ...
    "Flooding": [
        ...
        "submerged", "water stagnation", "water on road", "rainwater",
        # NOTE: bare "overflow" removed — too ambiguous; use compound forms below
        "overflow",
    ],
    "Streetlight": [
    ...
    "Waste": [
        # Compound phrases first — beat "overflow" in Flooding
        "overflowing garbage", "overflowing bins", "garbage bins",
        "overflowing dustbin", "bins overflowing",
    ...
    "Noise": [
        ...
        "disturbance", "racket", "drilling", "wedding band", "band playing",
    ],
    "Road Damage": [
        ...
        "road sunken", "road surface", "footpath", "pavement",
        "tiles broken", "footpath broken", "upturned paving", "paving",
        "road subsided", "cobblestones",
    ],
    "Heritage Damage": [
        "heritage", "monument", "historical", "ancient", "temple wall",
        "heritage site", "old building", "protected structure",
        "heritage structure", "historic",
    ],
    "Heat Hazard": [
    ...
    "Drain Blockage": [
        "overflow drain", "blocked drain", "drain block", "drain choke",
        "choked drain", "drain", "drainage",
        "sewer", "sewage", "manhole", "gutter", "nala",
    ],
}

# Sort each list longest-first so compound phrases match before short ones
CATEGORY_KEYWORDS: dict[str, list[str]] = {
    cat: sorted(kws, key=len, reverse=True)
    for cat, kws in _RAW_CATEGORY_KEYWORDS.items()
}
