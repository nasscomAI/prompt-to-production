...
Policy document summarizer that preserves all obligations and conditions.
"""
import argparse
import re
...
def summarize_policy(sections: list) -> str:
    output_lines = []
    output_lines.append("CITY MUNICIPAL CORPORATION")
    output_lines.append("HUMAN RESOURCES DEPARTMENT")
    output_lines.append("EMPLOYEE LEAVE POLICY — SUMMARY")
    output_lines.append("")
    ...
    for section in sections:
        ...
        group_num = section_num.split(".")[0]

        if group_num != current_group:
            current_group = group_num
            group_title = section_groups.get(group_num, "")
            output_lines.append("")
            output_lines.append(f"{group_num}. {group_title}")
            output_lines.append("")

        summary = CLAUSE_SUMMARIES.get(section_num, section["content"])
        output_lines.append(f"{section_num} {summary}")
