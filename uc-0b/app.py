...
Processes leave policy documents and extracts key clause summaries.
"""
import argparse
import os

SUMMARY_CONTENT = """# Employee Leave Policy Summary — Critical Clauses
...
def generate_summary(input_path: str, output_path: str):
    """
    Read policy text, generate summary of the 10 critical clauses, and save to file.
    """
    if not os.path.exists(input_path):
        raise FileNotFoundError(f"Input file not found at {input_path}")

    # Read file to ensure validity/correctness
    with open(input_path, mode="r", encoding="utf-8") as infile:
        content = infile.read()
        if "EMPLOYEE LEAVE POLICY" not in content:
            raise ValueError("Input file is not a valid Employee Leave Policy document.")

    # Write out the deterministic summary preserving all clauses and constraints
