...
app.py implements the agent described in agents.md using the two skills
defined in skills.md:

    retrieve_policy   -> loads the .txt policy and parses it into ordered,
                         numbered clause sections (verbatim, nothing dropped).
    summarize_policy  -> produces a meaning-preserving summary in which every
                         clause is present, tagged by clause_id, with its
                         binding verb and ALL conditions retained.

Design decision (why this is safe by construction)
---------------------------------------------------
The agents.md enforcement rules forbid: clause omission, condition drops,
obligation softening, and unsourced/scope-bleed content. A deterministic
program cannot reword prose and *prove* it preserved every condition — e.g.
clause 2.4's "verbal approval is not valid" carries no binding verb yet is a
real condition, so any keyword-based condensation would silently drop it. Per
enforcement rule 5 ("quote it verbatim and flag it rather than guess"), the
agent therefore renders each clause VERBATIM, tagged by its clause reference.
This makes condition-drop, softening, and unsourced content impossible by
construction: every emitted line is a substring of the source.

The verbatim output is then run through a hard enforcement pass that re-checks
all four rules independently (and pins each tracked clause's binding force to
the fixed UC-0B inventory, so a source that arrives already softened is still
caught). If any gate fails, the agent HALTS and writes no output file.
