skills:
  - name: retrieve_policy
    ...
    output: >
      list of dicts in source order, each with section (str, e.g. "2"),
  ...
  - name: summarize_policy
    ...
    error_handling: >
      Before writing, asserts that every clause number from the input appears in
      the rendered text, that every condition token in a binding clause survives,
      and that no scope-bleed phrase has entered. If any check fails it prints
