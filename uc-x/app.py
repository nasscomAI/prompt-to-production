...
def answer_question(index, question, idf):
    """
    ...
    Error handling:
      ...
      - Top-scoring sections from two different documents score too close
        together (genuine cross-document ambiguity) -> return refusal
        template verbatim rather than blend or guess.
    ...
    AMBIGUITY_RATIO = 0.7  # second doc within 70% of top doc's score = too close to call

    if top_score < MIN_SCORE:
        return REFUSAL_TEMPLATE, True

    if second_score >= top_score * AMBIGUITY_RATIO and second_score >= MIN_SCORE:
        # Genuine cross-document ambiguity — refuse rather than blend.
        return REFUSAL_TEMPLATE, True
...
def main():
    ...
    while True:
        ...
        answer, is_refusal = answer_question(index, question, idf)
        print()
        print(answer)
        print()
