...
> [UC-0B] Fix clause omission: completeness not enforced → added every-numbered-clause rule and implemented condition preservation
...
> [UC-0C] Fix silent aggregation: no scope in enforcement → restricted to per-ward per-category only and added formula transparency
...
> [UC-X] Fix cross-doc blending: no single-source rule → added single-source attribution and exact refusal template enforcement
